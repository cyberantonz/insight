...
| `/connector create` skill | Required output | Skill emits an `images:` block with one entry per Dockerfile the connector ships. Validated by `cfs validate` rule `connector-images-triad`. |
...
- **Bad**, the runtime semantics of the two reserved keys (`cdk`, `enrich`) are not enforced by the schema itself; a future ADR or cfs validate rule must check that any key reconcile/enrich expects is present and non-empty.
...
- Bad, because two keys (`cdk`, `enrich`) have runtime semantics not enforced by the schema. Mitigated by `cfs validate` rule `connector-images-triad`.
